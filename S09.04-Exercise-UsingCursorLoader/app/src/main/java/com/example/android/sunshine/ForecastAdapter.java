...
//      TODO (6) Move the cursor to the appropriate position
        ...
        {
            throw new IllegalStateException("Invalid Cursor object");
        }
        ...
        float minTemp = mCursor.getFloat(mCursor.getColumnIndex(WeatherContract.WeatherEntry.COLUMN_MIN_TEMP));
        float maxTemp = mCursor.getFloat(mCursor.getColumnIndex(WeatherContract.WeatherEntry.COLUMN_MAX_TEMP));

        //      TODO (7) Generate a weather summary with the date, description, high and low
        String weatherSummary =
                SunshineDateUtils.getFriendlyDateString(mContext, normalizedDate, true)
